...
- [ ] 1b.1 Run p2 patch on _p1 blobs — full merged scope: (a) dropped-bomb bomb_x/y from carrier position at has_c4 falling edge; (b) steamid/name meta enrichment; (c) `place` sidecar for the 81 local stems; **(d) crop end-phase frames beyond round_end + 7s; (e) clamp bomb_age at explosion/defuse (cap 1.0 normalized)** — [2]'s quantile fit must not ingest the tail frames. Same validation pattern as [1] (sample diff vs fixed-builder re-bake); lineage restamp; flip defaults _p1 → _p2 in the same commit → **DONE-CHECK: _p2 blobs + manifest entry + validation report committed**. *(rb [1b] + r2 §7 [1b] merged; corpus-audit §5.B.3)*
  ...
  - [x] `val_v2m_p2.pt` local artifact + manifest lineage — 770/770 rounds, post-write invariants and manifest sha PASS; 46 rounds / 7,294 excess tail frames removed; 10,507 bomb-age values clamped. Drop inference checked against 222 source-keyed local events: precision 0.982, recall 0.987, median xy error 1.22 game units (`validate_p2_drop_inference.py`). **Partial only; not canonical.**
  - [ ] Remaining for 1b.1: generate `val_v3m_p2.pt` and both train blobs on the quiet machine; run the required fresh-builder local-era + HF-era diff; flip live defaults together; only then set `p2_status.complete/canonical` and tick 1b.1.
