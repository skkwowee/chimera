...
| D8 | Half-boundary end-phase tail (found empirically 2026-07-26, deep-sweep-r2): r12 rounds carry up to ~60 s of dead halftime frames past round_end (non-boundary rounds pad exactly 7.0 s); on exactly those rounds `bomb_age` overflows its /40 normalization to 2.27×; r24 rounds are truncated to a 0 s end phase. `bomb_age` appeared in no register before this row. | Fix folded into the [1b] p2 patch: crop frames beyond round_end + 7 s; clamp bomb_age at 1.0. r24 truncation disclosed as-is. | Unpatched, [2]'s quantile edge fit ingests the tail frames; end-phase-masked losses already exclude them at train time, so the leak path is the edge fit + any unmasked eval. |
